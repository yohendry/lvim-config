return {
  on_attach = function(client)
    ...
    local exist_wk, wk = util.module_exist("which-key")
    if exist_wk then
      wk.register({
        t = {
          name = "Typescript",
          ...
          R = { "<cmd>TSToolsRemoveUnused<CR>", "Remove all unused references" },
          a = { "<cmd>TSToolsAddMissingImports<CR>", "Add missing Imports" },
          f = { "<cmd>TSToolsFixAll<CR>", "Fix all" },
          s = { "<cmd>TSToolsRenameFile<CR>", "File rename" },
        },
      }, { prefix = "<leader>l" })
      lvim.keys.normal_mode["gd"] = "<cmd>TSToolsGoToSourceDefinition<cr>"
    end
  end,
