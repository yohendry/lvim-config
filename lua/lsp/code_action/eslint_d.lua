...
-- local shorthen_eslint_rule = function(rule)
-- 	local parts = mysplit(rule, "/")

-- 	return parts[#parts]
-- end

local generate_fix_action = function(message, params)
	-- local rule_id = shorthen_eslint_rule(message.ruleId)
	local rule_id = message.ruleId
	local title = "󰁨  fix for  " .. rule_id
...
local generate_disable_actions = function(message, indentation, params)
	...
	local rule_id = message.ruleId
	local actions = {}
	local line_title = "󰜺 rule " .. rule_id .. " for this line"
	...
	local file_title = "󰜺 rule " .. rule_id .. " for the entire file"
...
return h.make_builtin({
	...
	generator_opts = {
		...
		cwd = h.cache.by_bufnr(function(params)
			return myutil.cosmiconfig("eslint", "eslintConfig")(params.bufname)
		end),
	},
